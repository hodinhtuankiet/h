/**
 * ---
 * @param {*} originalArray
 * @param {*} orderArray
 * @param {*} key = Key to order
 * @return new Ordered Array
 *
 * For Vietnamese with love :D
 * Xác định các phần tử trong array gốc ban đầu (originalArray) xem nó nằm ở đâu trong
 *  array thứ 2 (orderArray) (là array mà mình dùng để sắp xếp)
 *  bằng cách tìm index (indexOf) rồi sẽ sắp xếp theo index đó bằng hàm sort của Javascript.
 */

export const mapOrder = (originalArray, orderArray, key) => {
  if (!originalArray || !orderArray || !key) return []
  // spread-operator(...) bóc tách từng value trong mảng ra [1 , 2 , 3 ] -> 1 , 2 , 3
  const clonedArray = [...originalArray]
  // (a,b) sắp xếp 2 giá trị liền kề nhau
  const orderedArray = clonedArray.sort((a, b) => {
    // const beasts = ['ant', 'bison', 'camel', 'duck', 'bison'];
    // console.log(beasts.indexOf('bison'));
    // Expected output: 1
    return orderArray.indexOf(a[key]) - orderArray.indexOf(b[key])
  })

  return orderedArray
}
//   Brief
export const mapOrder2 = (originalArray, orderArray, key) => {
  if (!originalArray || !orderArray || !key) return []
  // spread-operator(...) bóc tách từng value trong mảng ra [1 , 2 , 3 ] -> 1 , 2 , 3
  return [...originalArray].sort((a, b) => orderArray.indexOf(a[key]) - orderArray.indexOf(b[key]))
}

/**
   * Example:
   */
const originalItems = [
  { id: 'id-1', name: 'One' },
  { id: 'id-2', name: 'Two' },
  { id: 'id-3', name: 'Three' },
  { id: 'id-4', name: 'Four' },
  { id: 'id-5', name: 'Five' }
]
// mảng này chứa danh sách của mảng originalItems
const itemOrderIds = ['id-5', 'id-4', 'id-2', 'id-3', 'id-1']
const key = 'id'

const orderedArray = mapOrder(originalItems, itemOrderIds, key)
console.log('Original:', originalItems)
console.log('Ordered:', orderedArray)
/**
   * Results:
   *
   * Original: Of course, nothing changes =))
   *
   * Ordered:
   *  [
        { id: 'id-5', name: 'Five' },
        { id: 'id-4', name: 'Four' },
        { id: 'id-2', name: 'Two' },
        { id: 'id-3', name: 'Three' },
        { id: 'id-1', name: 'One' }
    * ]
   */